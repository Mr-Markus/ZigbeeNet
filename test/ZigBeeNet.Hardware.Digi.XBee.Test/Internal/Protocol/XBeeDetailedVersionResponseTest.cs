...
{
    public class XBeeDetailedVersionResponseTest : XBeeFrameBaseTest
    {
        private readonly ITestOutputHelper _output;

        public XBeeDetailedVersionResponseTest(ITestOutputHelper output)
        {
            _output = output;
        }
        ...
        {
            XBeeDetailedVersionResponse responseEvent = new XBeeDetailedVersionResponse();
            responseEvent.Deserialize(GetPacketData(
            ...
            Assert.Equal(6, responseEvent.GetFrameId());
            Assert.Equal(CommandStatus.OK, responseEvent.GetCommandStatus());

            int[] arrayToTestAgainst = GetPacketData(
            ...
            int[] versionInfoResult = responseEvent.GetVersionInfo();

            Assert.True(arrayToTestAgainst.SequenceEqual(versionInfoResult));
        }
    }
}
