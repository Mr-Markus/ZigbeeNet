    ...
    public static class ZclProtocolCodeGenerator
    {
        ...
        private static void CompareFiles(string inFolder, string outFolder, string folder)
        {
            ...
            foreach (var file in files)
            {
                ...
                try
                {
                    if (!FileCompare(inFolder + folder + "/" + file.Name,
                            outFolder + folder + "/" + file.Name))
                    {
        ...
        public static void Generate(string[] args = null)
        {
            ...
            _generatedDate = DateTime.UtcNow.Date.ToString() + " - " + DateTime.UtcNow.ToShortTimeString();
            ...
            //FileInfo packageFolder = new FileInfo(sourceRootPath);
            ...
            //{
            //    Console.WriteLine("Source root path does not exist: " + sourceRootFile);
            //    return;
            //}

            //if (!sourceRootFile.isDirectory())
            //{
            //    Console.WriteLine("Source root path is not directory: " + sourceRootFile);
            //    return;
            //}
                ...
                foreach (Cluster cluster in clusters)
                {
                    ...
                    foreach (Command command in commands)
                    {
                        ...
                        foreach (Field field in fields)
                        {
                            ...
                            code.AppendLine("     */");
                            code.AppendLine("    private " + field.DataTypeClass + " " + field.NameLowerCamelCase + ";");
                            code.AppendLine();
                        }
                            ...
                            foreach (Field field in fields)
                            {
                                ...
                                if (field.ListSizer != null)
                                {
                                    if (field.ListSizer.Equals("statusResponse"))
                                    {
                                        ...
                                        code.AppendLine("        {");
                                        code.AppendLine("            serializer.serialize(status, ZclDataType.ZCL_STATUS);");
        ...
        private static void GenerateZclClusterClasses(Context context/*, string packageRootPrefix, File sourceRootPath*/)
        {
            ...
            foreach (Profile profile in profiles)
            {
                ...
                foreach (Cluster cluster in clusters)
                {
                    ...
                    code.AppendLine();
                    code.AppendLine("/**");
                    code.AppendLine(" *" + cluster.ClusterName + "cluster implementation (Cluster ID 0x" + cluster.ClusterId.ToString("X") + ").");
                    ...
                    if (cluster.Attributes.Count != 0)
                    {
                        ...
                        foreach (Attribute attribute in cluster.Attributes.Values)
                        {
                            code.AppendLine("        attributeMap.put(" + attribute.EnumName
                                    ...
                                    + attribute.AttributeAccess.ToLower().Contains("read") + ", "
                                    + attribute.AttributeAccess.ToLower().Contains("write") + ", "
                                    + "mandatory".Equals(attribute.AttributeReporting.ToLower()) + "));");
                        }
                    }
                    code.AppendLine();
