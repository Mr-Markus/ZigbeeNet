...
namespace ZigBeeNet.ZCL.Clusters
{
   public class ZclIASACECluster : ZclCluster
   {
       ...
       * This command is used by ACE clients to request an update of the status of the IAS Zone devices managed by the ACE server       * (i.e., the IAS CIE). In particular, this command is useful for battery-powered ACE clients with polling rates longer than       * the ZigBee standard check-in rate. The command is similar to the Get Attributes Supported command in that it specifies a       * starting Zone ID and a number of Zone IDs for which information is requested. Depending on the number of IAS Zone devices       * managed by the IAS ACE server, sending the Zone Status of all zones MAY not fit into a single Get ZoneStatus Response command.       * IAS ACE clients MAY need to send multiple Get Zone Status commands in order to get the information they seek.       </summary>
       ...
       <param name= armNotification {@link byte} Arm Notification</param>
       <returns the Task<CommandResult> command result Task
       </returns>
       ...
       * The 16 fields of the payload indicate whether each of the Zone IDs from 0 to 0xff is allocated or not. If bit n       * of Zone ID Map section N is set to 1, then Zone ID (16 x N + n ) is allocated, else it is not allocated       </summary>
       <param name= zoneIDMapSection0 {@link ushort} Zone ID Map section 0</param>
       <param name= zoneIDMapSection1 {@link ushort} Zone ID Map section 1</param>
       <param name= zoneIDMapSection2 {@link ushort} Zone ID Map section 2</param>
       <param name= zoneIDMapSection3 {@link ushort} Zone ID Map section 3</param>
       <param name= zoneIDMapSection4 {@link ushort} Zone ID Map section 4</param>
       <param name= zoneIDMapSection5 {@link ushort} Zone ID Map section 5</param>
       <param name= zoneIDMapSection6 {@link ushort} Zone ID Map section 6</param>
       <param name= zoneIDMapSection7 {@link ushort} Zone ID Map section 7</param>
       <param name= zoneIDMapSection8 {@link ushort} Zone ID Map section 8</param>
       <param name= zoneIDMapSection9 {@link ushort} Zone ID Map section 9</param>
       <param name= zoneIDMapSection10 {@link ushort} Zone ID Map section 10</param>
       <param name= zoneIDMapSection11 {@link ushort} Zone ID Map section 11</param>
       <param name= zoneIDMapSection12 {@link ushort} Zone ID Map section 12</param>
       <param name= zoneIDMapSection13 {@link ushort} Zone ID Map section 13</param>
       <param name= zoneIDMapSection14 {@link ushort} Zone ID Map section 14</param>
       <param name= zoneIDMapSection15 {@link ushort} Zone ID Map section 15</param>
       <returns the Task<CommandResult> command result Task
       </returns>
       ...
       <param name= zoneID {@link byte} Zone ID</param>
       <param name= zoneType {@link ushort} Zone Type</param>
       <param name= iEEEAddress {@link IeeeAddress} IEEE address</param>
       <param name= zoneLabel {@link string} Zone Label</param>
       <returns the Task<CommandResult> command result Task
       </returns>
       ...
       * This command updates requesting IAS ACE clients in the system of changes to the security panel status recorded by       * the ACE server (e.g., IAS CIE device).       </summary>
       <param name= panelStatus {@link byte} Panel Status</param>
       <param name= secondsRemaining {@link byte} Seconds Remaining</param>
       <param name= audibleNotification {@link byte} Audible Notification</param>
       <param name= alarmStatus {@link byte} Alarm Status</param>
       ...
       * Sets the list of bypassed zones on the IAS ACE client. This command can be sent either as a response to the       * GetBypassedZoneList command or unsolicited when the list of bypassed zones changes on the ACE server.       </summary>
       <param name= zoneID {@link List<byte>} Zone ID</param>
       <returns the Task<CommandResult> command result Task
       </returns>
       ...
       * Provides the response of the security panel to the request from the IAS ACE client to bypass zones via a Bypass command.       </summary>
       <param name= bypassResult {@link List<byte>} Bypass Result</param>
       <returns the Task<CommandResult> command result Task
       </returns>
       ...
       * This command updates requesting IAS ACE clients in the system of changes to the IAS Zone server statuses recorded       * by the ACE server (e.g., IAS CIE device).       </summary>
       <param name= zoneStatusComplete {@link bool} Zone Status Complete</param>
       <param name= numberOfZones {@link byte} Number of zones</param>
       <param name= iasAceZoneStatus {@link byte} Ias Ace Zone Status</param>
       <param name= zoneId {@link byte} Zone Id</param>
       <param name= zoneStatus {@link ushort} Zone Status</param>
       <returns the Task<CommandResult> command result Task
       </returns>
